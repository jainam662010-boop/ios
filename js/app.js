...
logout(){
 ...
 timerHTML(){
   return `<div class="timer-widget au-4" id="timerWidget" style="position:fixed; bottom:32px; left:32px; z-index:1000">
     <div class="premium-card timer-card" id="timerCard" style="padding:16px; display:flex; align-items:center; gap:12px; min-width:200px; transition:all 0.2s ease-out;">
       <div id="timerIcon" style="font-size:24px">⏱️</div>
 ...
 toggleMinimizeTimer(){
   ...
   const btn = document.getElementById('timerMinimizeBtn');
   ...
   if (this._timerMinimized) {
     ...
     widget.onclick = (e) => {
       if (e.target.closest('#timerBtn')) return;
       this.toggleMinimizeTimer();
     };
   } else {
...
initPage(active){
  ...
  // Remove old backdrop and use new one
